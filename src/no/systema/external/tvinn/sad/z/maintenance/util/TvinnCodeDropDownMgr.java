 ...
 * 
 */
package no.systema.external.tvinn.sad.z.maintenance.util;
...
import no.systema.external.tvinn.sad.z.maintenance.service.MaintSadFellesKodtsiService;
import no.systema.external.tvinn.sad.z.maintenance.model.JsonMaintNctsTransitKodeTypeContainer;
import no.systema.external.tvinn.sad.z.maintenance.model.JsonMaintNctsTransitKodeTypeRecord;
import no.systema.external.tvinn.sad.z.maintenance.service.MaintNctsExportTrkodfService;
import no.systema.external.tvinn.sad.z.maintenance.model.JsonMaintSadExportKodts6Container;
import no.systema.external.tvinn.sad.z.maintenance.model.JsonMaintSadExportKodts6Record;
 ...
 * @date Sep 15, 2016 / Delameluri
 * 
 * 	
 */

public class TvinnCodeDropDownMgr {
	private static final Logger logger = Logger.getLogger(TvinnCodeDropDownMgr.class.getName());
	...
	public void populatePrefCodesHtmlDropDownsSadExport(UrlCgiProxyService urlCgiProxyService, MaintSadExportKodts6Service maintSadExportKodts6Service, Map model, String applicationUser ){
		String BASE_URL = TvinnSadMaintenanceUrlDataStore.TVINN_SAD_MAINTENANCE_EXPORT_BASE_SAD002_KODTS6R_GET_LIST_URL;
	...
	public void populateTransitKoder(UrlCgiProxyService urlCgiProxyService, MaintNctsExportTrkodfService maintNctsExportTrkodfService, Map model,
		...
		String BASE_URL = TvinnSadMaintenanceUrlDataStore.TVINN_NCTS_MAINTENANCE_EXPORT_BASE_TR001R_GET_KODE_TYPER_URL;
		StringBuffer urlRequestParams = new StringBuffer();
		urlRequestParams.append("user="+ applicationUser);
	...
	public void populateCodesHtmlDropDownsFromJsonSignature(UrlCgiProxyService urlCgiProxyService, MaintSadFellesKodtsiService maintSadFellesKodtsiService, Map model, SystemaWebUser appUser) {
		String BASE_URL = TvinnSadMaintenanceUrlDataStore.TVINN_SAD_MAINTENANCE_FELLES_BASE_SYFT10R_GET_LIST_URL;
		String urlRequestParams = "user=" + appUser.getUser();
		logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		logger.info("URL: " + jsonDebugger.getBASE_URL_NoHostName(BASE_URL));
		logger.info("URL PARAMS: " + urlRequestParams);
