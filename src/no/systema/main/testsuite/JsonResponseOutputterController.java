...
public class JsonResponseOutputterController {
	...
	/**
	 * Test call 
	 * http://localhost:8080/espedsg2/sytsuite_transpDisp.do?user=OSCAR
	 * @return
	 */
	@RequestMapping(value="/sytsuite_transpDisp.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String sytsuite_transpDisp( HttpSession session, HttpServletRequest request) {
		StringBuffer sb = new StringBuffer();
		try{
			logger.info("Inside sytsuite_transpDisp");
			//TEST-->logger.info("Servlet root:" + AppConstants.VERSION_SYJSERVICES);
			
			//get list
			logger.info("Init lists");
			List<Object> urlStoreObjectList = this.getUrlObjectListTranspDisp();
	 ...
	 * @return
	 */
	@RequestMapping(value="/sytsuite_sporroppd.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String sytsuite_sporroppd( HttpSession session, HttpServletRequest request) {
		StringBuffer sb = new StringBuffer();
		try{
			logger.info("Inside sytsuite_sporroppd");
			//TEST-->logger.info("Servlet root:" + AppConstants.VERSION_SYJSERVICES);
			
			//get list
			logger.info("Init lists");
			List<Object> urlStoreObjectList = this.getUrlObjectListSporrOppd();
			List<TestersuiteObject> testersuiteObjectlist = new ArrayList<TestersuiteObject>(); 
			for(Object urlStoreObjRecord : urlStoreObjectList){
				logger.info("testing urlStore:" + urlStoreObjRecord.getClass().getName());
				reflectionMgr.buildList(testersuiteObjectlist, urlStoreObjRecord);
