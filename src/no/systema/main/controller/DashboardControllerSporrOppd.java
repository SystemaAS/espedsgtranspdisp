...
/**
 * Dashboard for Systema Web Sporr.Oppdrag
 * 
...
@Controller
@Scope("session")
public class DashboardControllerSporrOppd {
	private static final Logger logger = LoggerFactory.getLogger(DashboardControllerSporrOppd.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="logonDashboard_sporroppd.do", method= { RequestMethod.POST, RequestMethod.GET })
	public ModelAndView logon(@ModelAttribute (AppConstants.SYSTEMA_WEB_USER_KEY) SystemaWebUser appUser, BindingResult bindingResult, HttpSession session, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttr){
		...
		logger.info("Inside logon SPORR-OPPD...");
	 ...
	 * @return
	 */
	@RequestMapping(value="logonWRedDashboard_sporroppd.do", method= { RequestMethod.POST, RequestMethod.GET})
	...
	private String getTomcatServerRedirectionUrl(SystemaWebUser appUser, HttpServletRequest request){
		...
		try{
			retval = hostRaw + request.getContextPath() + "/logonWRedDashboard_sporroppd.do?" + "lang=" + appUser.getUsrLang() + "&ru=" + URLEncoder.encode(appUser.getEncryptedUser(), "UTF-8") + "&dp=" + URLEncoder.encode(appUser.getEncryptedPassword(), "UTF-8");
		}catch(Exception e){
	...
	private void doHandOverToSystemaWebUser(HttpServletRequest request, SystemaWebUser appUser, JsonSystemaUserContainer jsonSystemaUserContainer, String companyCode){
		...
		*/
		...
		appUser.setServletHost(AppConstants.HTTP_PROTOCOL + "://" + host);
